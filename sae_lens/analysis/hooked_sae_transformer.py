"""This code is mostly borrowed from a TransformerLens PR Written by Connor Kissane."""
...
class InferenceSparseAutoencoder(SparseAutoencoder):
    """A Wrapper around the Sparse Autoencoder that only returns the SAE output,
    rather than the full output of the forward pass. This is useful for inference,
    when you only want the SAE output other variables you care about are retrieved
    through the HookedSAETransformer cache.
    """

    def forward(  # type: ignore (We are intentionally overriding the return type of the forward method)
        self, input: torch.Tensor, *model_args: Any, **model_kwargs: Any
    ) -> torch.Tensor:

        # usually the input is the output of the previous layer
        if input.shape[-1] == self.cfg.d_in:
            return (
                super(InferenceSparseAutoencoder, self)
                .forward(input, *model_args, **model_kwargs)
                .sae_out
            )

        # however for attention outputs, the input is the output of the attention layer which is a 3D tensor
        # the activation store will usually deal with this but when using the SAE inside a HookeSAETransformer
        # we need to handle this case.
        else:
            # Assume this this is an attention output (hook_z) SAE
            assert self.cfg.hook_point.endswith(
                "_z"
            ), f"You passed in an input shape {input.shape} does not match SAE input size {self.cfg.d_in} for hook_name {self.cfg.hook_point}. This is only supported for attn output (hook_z) SAEs."
            d_head = input.shape[-1]
            x = einops.rearrange(input, "... n_heads d_head -> ... (n_heads d_head)")
            x_reconstructed = (
                super(InferenceSparseAutoencoder, self)
                .forward(x, *model_args, **model_kwargs)
                .sae_out
            )
    ...
    @classmethod
    def from_sparse_autoencoder(
        cls, sae: SparseAutoencoder
    ) -> "InferenceSparseAutoencoder":
        inference_sparse_autoencoder = InferenceSparseAutoencoder(sae.cfg)
        inference_sparse_autoencoder.load_state_dict(sae.state_dict())
...
class HookedSAETransformer(HookedTransformer):
    def __init__(
        ...
        **model_kwargs: Any,
        ...
        self.acts_to_saes: Dict[str, InferenceSparseAutoencoder] = {}

    def attach_sae(
        self,
        sae: SparseAutoencoder,
        turn_on: bool = True,
        hook_name: Optional[Union[str, List[str]]] = None,
    ):
        ...
        self.acts_to_saes[act_name] = (
            InferenceSparseAutoencoder.from_sparse_autoencoder(sae)
        )
        if turn_on:
    ...
    def run_with_cache_with_saes(
        ...
        **kwargs: Any,
        ...
        try:
            self.turn_saes_on(act_names)
            out = self.run_with_cache(  # type: ignore
                *model_args,
