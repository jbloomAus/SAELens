...
def cache_activations_runner(cfg: CacheActivationsRunnerConfig):
    ...
    # Activation Store currently assumes it's getting the path to already cached activations
    # we should find a nicer way to deal with this.
    ...
    if os.path.exists(cfg.new_cached_activations_path):
        if len(os.listdir(cfg.new_cached_activations_path)) > 0:
            raise Exception(
                f"Activations directory ({cfg.new_cached_activations_path}) is not empty. Please delete it or specify a different path. Exiting the script to prevent accidental deletion of files."
            )
    else:
        os.makedirs(cfg.new_cached_activations_path)
    ...
    for i in range(n_buffers):
        ...
        torch.save(buffer, f"{cfg.new_cached_activations_path}/{i}.pt")
        del buffer
        ...
        if i % cfg.shuffle_every_n_buffers == 0 and i > 0:
            ...
            for _ in range(cfg.n_shuffles_with_last_section):
                shuffle_activations_pairwise(
                    cfg.new_cached_activations_path,
