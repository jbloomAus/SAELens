...
def build_transcoder_cfg(**kwargs: Any) -> TranscoderConfig:
    ...
    return TranscoderConfig(**{**default_config, **kwargs})
...
class TestTranscoder:
    ...
    def test_transcoder_get_output_hook_name(self):
        ...
        cfg_with_head = build_transcoder_cfg(
            hook_name_out="blocks.1.attn.hook_z",
            hook_head_index_out=3
        )
        transcoder_with_head = Transcoder(cfg_with_head)
        assert (transcoder_with_head.get_output_hook_name() == 
                "blocks.1.attn.hook_z.h3")
    ...
    def test_transcoder_fold_w_dec_norm(self):
        ...
        x = torch.randn(10, cfg.d_in)
        out1, feat1 = transcoder(x)
        out2, feat2 = transcoder2(x)
...
class TestJumpReLUTranscoder:
    ...
    def test_jumprelu_transcoder_encode(self):
        ...
        transcoder.W_enc.data = (
            torch.eye(cfg.d_in, cfg.d_sae)[:cfg.d_in, :cfg.d_sae]
        )
